...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const pList = snapshot.docs.map((doc, idx) => ({
        id: doc.id,
        number: (snapshot.docs.length - idx).toString().padStart(2, '0'),
        ...doc.data()
      })) as Project[];
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F3] text-brand-green selection:bg-brand-green selection:text-white">
      ...
      <section className="pt-24 pb-20 px-6 md:px-10">
        <div className="max-w-screen-2xl mx-auto">
          ...
          >
            ...
            <h1 className="text-3xl md:text-[6vw] font-display leading-[0.85] tracking-tighter uppercase mb-2 relative inline-block">
      ...
      <section className="pb-40 min-h-[400px]">
        {loading ? (
          <div className="flex justify-center p-20 opacity-20 animate-pulse font-mono text-[10px] uppercase tracking-widest">Loading...</div>
        ) : !selectedCategory ? (
          // Category List
                ...
                >
                  ...
                  <div className="relative z-10 flex items-center justify-between py-10 px-6 md:px-12 transition-transform duration-500 group-hover:translate-x-4">
                    <div className="flex items-baseline gap-8 md:gap-16">
                      <div className="flex flex-col">
                        <h2 className="text-2xl md:text-5xl lg:text-6xl font-display font-black leading-none tracking-tighter transition-all duration-500">
                          {cat}
                        </h2>
                      </div>
                    </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-12 md:gap-20">
            ...
            <div className="lg:col-span-4 space-y-6">
              <button 
                ...
                className="font-display text-3xl md:text-4xl tracking-tighter hover:opacity-80 transition-opacity text-left"
              >
                DABIN GROUP
              </button>
